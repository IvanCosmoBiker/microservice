...
func InitCore(pathCert string, debug bool) *Core {
	return &Core{
		PathCert: pathCert,
	}
}

func (c *Core) MakeRequestSendCheck(tran *transaction.Transaction) (string, string, []byte, error) {
	var signature string
	var jsonRequest []byte
	var err error
	if tran.Fiscal.OnlyFiscal {
		return tran.Fiscal.Signature, tran.Fiscal.ResiptId, tran.Fiscal.FiscalRequest.Request, err
	}
	...
	if tran.Fiscal.Config.Ffd_version == 2 {
		request.Content.FsItemCodeType = "0"
		request.IgnoreItemCodeCheck = true
	}
	jsonRequest, err = json.Marshal(request)
	if err != nil {
		request = nil
		return signature, id, []byte(""), err
	}
	if tran.Fiscal.Config.Type != fr.Fr_EphorServerOrangeData && tran.Fiscal.Config.Type != fr.Fr_EphorOrangeData {
		signature, err = c.ComputeSignature(jsonRequest, []byte(tran.Fiscal.Config.Sign_private_key))
		return signature, id, jsonRequest, err
	}
...
func (c *Core) Pack(src []byte) []byte {
	dst := make([]byte, hex.EncodedLen(len(src)))
	hex.Encode(dst, src)
	return dst
}

func (c *Core) ComputeSignature(data, privatesignKey []byte) (signature string, err error) {
	pack := c.Pack([]byte("3031300d060960864801650304020105000420"))
	...
	h.Write(buf.Bytes())
	hash := h.Sum(nil)
	result, err := c.RsaEncrypt(hash, privatesignKey)
	if err != nil {
		buf = nil
		hash = nil
		h = nil
	...
	for _, product := range tran.Products {
		...
		if tran.Fiscal.Config.Ffd_version == 2 {
			if product.Marking != 0 {
				entryPositions.ItemCode = fmt.Sprintf("0%s", product.Barcode)
				entryPositions.PaymentSubjectType = 33
				entryPositions.QuantityMeasurementUnit = "0"
				entryPositions.PlannedStatus = 2
				entryPositions.IndustryAttribute = &orangeRequestCheck.IndustryAttribute{
					FoivId:              "030",
					CauseDocumentDate:   "16.06.2022",
					CauseDocumentNumber: "174",
					Value:               "crpt=mrk&mode=vend",
				}
			} else {
...
func (c *Core) ReadFileCert() (string, error) {
	crtFilte := fmt.Sprintf("%s%s", c.PathCert, "/ephorOrangeData.crt")
...
func (c *Core) ReadFileKey() (string, error) {
	keyFile := fmt.Sprintf("%s%s", c.PathCert, "/ephorOrangeData.key")
