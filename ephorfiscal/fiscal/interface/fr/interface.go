...
var (
	...
	Status_Retry_Status uint8 = 13 // повторно опросить статус
)
...
// -- Error Fiscal -- //
var (
	Status_Fr_InQueue  = 101 // чек добавлен в очередь, реквизиты не получены
...
type Fiscal interface {
	SendCheck(req *pb.Request) map[string]interface{}
	GetStatus(req *pb.Request) map[string]interface{}
	GetQrPicture(req *pb.Request) string
	GetQrUrl(req *pb.Request) string
}
