...
import (
	...
	fiscalManager "listeners/fiscalmanager"
)
	const (
...
func initPayCron(forever chan bool) {
	stringQueue := cfg.Services.EphorFiscal.NameQueue
	...
	for d := range msg {
		...
		if err2 != nil {
			...
			log.Println(errData)
			ConnDb.AddLog(dataLog, "EphorFiscal", fmt.Sprintf("%s", err2),"EphorFiscalError")
		...
		ConnDb.AddLog(dataLog,"EphorFiscal", " ",req.D)
...
func Start(conf *configEphor.Config, Rabbit *ConnectionRabbitMQ.ChannelMQ, Db connectionPostgresql.DatabaseInstance,exit chan bool,reconnectRabbit chan bool,transactions transactionDispetcher.TransactionDispetcher) {
	fmt.Println("Start EphorFiscal...")
	ConnectionRabbit = Rabbit
	ConnDb = Db
	cfg = *conf
	Transactions = transactions
	go fiscalManager.StartListener(conf,Db,Rabbit,forever,Transactions)
	start(forever,reconnectRabbit)
}
