...
func (db *DatabaseInstance) Set( table string, parametrs map[string]interface{}, where map[string]interface{}) {
	...
	if err  != nil{
		...
		db.SetLog(fmt.Sprintf("%s",parametrs))
	}
}

func (db *DatabaseInstance) Add(table string, parametrs map[string]interface{}, where map[string]interface{}) int{
	var id int
	StringDataSet,WhereString :=db.BuildString(parametrs,where,"insert")
	log.Printf(" [data] %s", StringDataSet)
	log.Printf(" [where] %s", WhereString)
	stringConnection := fmt.Sprintf("INSERT INTO main.%s %s VALUES (%s) RETURNING id",table,StringDataSet,WhereString)
	log.Printf(" [x] %s", stringConnection)
	err:= db.Conn.QueryRow(context.Background(),stringConnection).Scan(&id)
	if err !=nil {
		log.Println(err)
	}
	return id
